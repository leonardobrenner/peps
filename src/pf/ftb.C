...
void FTB::write_jit()
{
  ...
  peps_name jit_file_name;    // jit file name in the ./jit directory
  ...
  system("g++ -fPIC -c -o jit/peps_jit.o jit/peps_jit.C");       // compile peps_jit.C
  system("g++ -fPIC -shared -o jit/peps_jit.so jit/peps_jit.o"); // generate .so library
...
//  jit_mat = new (rp (*)(const int *))[size()];       // don't work with g++ 3.4.4
