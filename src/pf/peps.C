...
//                                                                                    //
//====================================================================================//
//  This File:   peps.C                     Language: C++  (xlC and CC)               //
...
//====================================================================================//
void Compile_SAN(bool aggr_too, bool lump_too, bool norm_too)   //   Main Operation   //
//====================================================================================//
{                          //     SAN model name in all directories handled by PEPS
 ...
 if (Permission_to_Proceed(des_level_name, des_file)) {              // .san compilation
   ...
   if (Permission_to_Proceed(dsc_level_name, dsc_file)) {            // .des compilation
     ...
     T.stop();
     if (aggr_too || lump_too) {                                     // aggregation or lump
       if (aggr_too)
       ...
       if (Permission_to_Proceed(agg_level_name, dsc_file)) {
         ...
         if(aggr_too)
           ...
           DSC::dsc.aggregate_replics(replic_map, agg_level_name);
         T.stop();
       }
     }
     else
...
//====================================================================================//
   case compile         : Compile_SAN(false, false, false); break;
   case comp_aggr       : Compile_SAN(true,  false, false); break;
   case comp_norm       : Compile_SAN(false, false, true ); break;
   case comp_aggr_norm  : Compile_SAN(true,  false, true ); break;
   case comp_lump_norm  : Compile_SAN(false, true,  true ); break;
...
//====================================================================================//
   ...
   case unifor_std   : if (Start_Up_CND(buf))      Uniformization_CND(buf, 
                                                     Ask_Time_Transient_Solution());
                                                   Integrate_Results(true);
                       break;
